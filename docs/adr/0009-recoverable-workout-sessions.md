...
Temporary transport failures retain a pending device copy and retry on demand or when the app becomes active. A repeated mutation identifier returns the acknowledged server state, while a distinct stale mutation receives a conflict and never overwrites the newer state. A revision conflict never silently chooses a winner: the device copy remains intact until the user explicitly elects to load the server copy. Server-copy recovery loads the session by stable identifier, so it works whether the other device left it active or completed it; accepting a completed copy opens its ordinary summary. There is no automatic merge or last-write-wins behavior. Completion may happen offline and is synchronized later. Starting and discarding remain online operations. A completed session becomes immutable in this phase, and the device retains it until completion is acknowledged.
